...
const stats=()=>{const a=[...orders.values()],p=a.filter(o=>["paid","captured","authorised"].includes(o.status));return{totalOrders:a.length,paidOrders:p.length,pending:a.filter(o=>o.status==="pending").length,failed:a.filter(o=>o.status==="failed").length,revenue:p.reduce((s,o)=>s+o.amount,0),providers:{tabby:a.filter(o=>o.provider==="tabby").length,tamara:a.filter(o=>o.provider==="tamara").length}}};
...
app.post("/api/checkout",(req,res)=>{
  ...
  if(MODE!=="live"){
    order.providerOrderId=id(data.provider);order.status="paid";
  ...
  return res.status(501).json({error:"LIVE_PROVIDER_NOT_CONFIGURED",message:"Configure provider credentials and the verified provider adapter before enabling live mode."});
});
...
function recordWebhook(provider,req,res){
  ...
  const b=req.body||{},ref=b.order_reference_id||b.reference_id||b.order_reference||b.order_id;
  const order=[...orders.values()].find(o=>o.referenceId===ref||o.providerOrderId===ref||o.id===ref);
  if(order){const e=String(b.event_type||b.event||b.status||"").toLowerCase();
